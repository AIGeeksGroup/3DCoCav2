...
from utils.llm_judge import select_best_caption
...
class CoCa_3D(nn.Module):
    ...
    def __init__(
        ...
        self.text_seq_len = getattr(config, "text_seq_len", 77)
    ...
    def sample_captions_from_logits(
        self,
        ...
        probs = torch.softmax(text_logits / max(temperature, 1e-6), dim=-1)
        batch, seq_len, vocab_size = probs.shape
        candidates = [[] for _ in range(batch)]
            ...
            for _ in range(num_candidates):
                ...
                for t in range(seq_len):
                    ...
                    if top_k is not None and top_k > 0 and top_k < vocab_size:
                        topk_vals, topk_idx = torch.topk(dist, k=top_k)
                        dist = topk_vals / topk_vals.sum()
                        token_id = topk_idx[torch.multinomial(dist, 1)].item()
                    else:
                        token_id = torch.multinomial(dist, 1).item()
                    sampled_ids.append(token_id)
                candidates[b].append(tokenizer.decode(sampled_ids))
        return candidates

    def _get_special_tokens(self, tokenizer):
        bos_id = getattr(tokenizer, "bos_token_id", None)
        eos_id = getattr(tokenizer, "eos_token_id", None)
        if bos_id is None and hasattr(tokenizer, "encoder"):
            bos_id = tokenizer.encoder.get("<|startoftext|>")
        if eos_id is None and hasattr(tokenizer, "encoder"):
            eos_id = tokenizer.encoder.get("<|endoftext|>")
        if bos_id is None:
            bos_id = 0
        if eos_id is None:
            eos_id = bos_id
    ...
    def _decode_tokens(self, tokenizer, token_ids, eos_id):
        tokens = []
        for tid in token_ids:
            if tid == eos_id:
                break
            tokens.append(int(tid))
        return tokenizer.decode(tokens) if hasattr(tokenizer, "decode") else " ".join(map(str, tokens))

    def _uses_scanrefer_tokens(self) -> bool:
        return (
            getattr(self.tokenizer, "bos_token", None) == "sos"
            and getattr(self.tokenizer, "eos_token", None) == "eos"
        )

    def _normalize_caption_for_eval(self, caption: str) -> str:
        caption = caption.strip()
        if self._uses_scanrefer_tokens():
            if caption.startswith("sos "):
                caption = caption[4:]
            if caption.endswith(" eos"):
                caption = caption[:-4]
            caption = caption.strip()
            return f"sos {caption} eos" if caption else "sos eos"
        return caption
    ...
    def generate_captions_autoregressive(
        ...
        bos_id, eos_id = self._get_special_tokens(tokenizer)
        pad_id = 0

        def _greedy_decode():
            tokens = torch.full((batch, max_len), pad_id, dtype=torch.long, device=device)
            tokens[:, 0] = bos_id
            finished = torch.zeros((batch,), dtype=torch.bool, device=device)
            for t in range(1, max_len):
                logits = self.coca(text=tokens, image_tokens=image_tokens, return_loss=False)
                next_token = logits[:, t - 1].argmax(dim=-1)
                tokens[:, t] = next_token
                finished |= next_token.eq(eos_id)
                if torch.all(finished):
                    break
            return [self._decode_tokens(tokenizer, row.tolist(), eos_id) for row in tokens]

        if tts_config is None or tts_config.get("num_candidates", 1) <= 1:
            return _greedy_decode()

        num_candidates = tts_config.get("num_candidates", 8)
        temperature = tts_config.get("temperature", 1.0)
        top_k = tts_config.get("top_k", 50)

        all_candidates = [[] for _ in range(batch)]
        for _ in range(num_candidates):
            tokens = torch.full((batch, max_len), pad_id, dtype=torch.long, device=device)
            tokens[:, 0] = bos_id
            finished = torch.zeros((batch,), dtype=torch.bool, device=device)
            for t in range(1, max_len):
                logits = self.coca(text=tokens, image_tokens=image_tokens, return_loss=False)
                step_logits = logits[:, t - 1] / max(temperature, 1e-6)
                if top_k is not None and top_k > 0 and top_k < step_logits.shape[-1]:
                    topk_vals, topk_idx = torch.topk(step_logits, k=top_k)
                ...
                if torch.all(finished):
                    break
            for b in range(batch):
                all_candidates[b].append(self._decode_tokens(tokenizer, tokens[b].tolist(), eos_id))

        if summaries is None:
            return [cands[0] for cands in all_candidates]
        return [
            self.select_tts_caption(summaries[i], all_candidates[i], tts_config)
            for i in range(batch)
        ]

    def build_scene_summary(self, box_outputs, topk: int = 10):
        sem_probs = box_outputs["sem_cls_prob"]
        obj_probs = box_outputs["objectness_prob"]
        batch_size, num_queries, _ = sem_probs.shape

        summaries = []
        for b in range(batch_size):
            scores = obj_probs[b] * sem_probs[b].max(dim=-1).values
            k = min(topk, num_queries)
            topk_idx = torch.topk(scores, k=k).indices
            topk_cls = sem_probs[b, topk_idx].argmax(dim=-1).tolist()
            counts = {}
            for cls_id in topk_cls:
                name = self.dataset_config.class2type.get(cls_id, f"class_{cls_id}")
                counts[name] = counts.get(name, 0) + 1
            summary = "Objects: " + ", ".join([f"{k} x{v}" for k, v in counts.items()])
            summaries.append(summary)
        return summaries

    def score_caption(self, summary: str, caption: str):
        summary_tokens = set(re.findall(r"[a-z0-9]+", summary.lower()))
        caption_tokens = set(re.findall(r"[a-z0-9]+", caption.lower()))
        if not caption_tokens:
            return -1.0
        overlap = len(summary_tokens & caption_tokens)
        coverage = overlap / (len(caption_tokens) + 1e-6)
        length_penalty = min(len(caption_tokens) / 20.0, 1.0)
        return coverage * 0.7 + length_penalty * 0.3

    def select_tts_caption(self, summary: str, candidates, judge_config=None):
        judge = (judge_config or {}).get("judge", "heuristic")
        if judge == "llm":
            base_url = judge_config.get("judge_base_url", "")
            model = judge_config.get("judge_model", "")
            api_key = judge_config.get("judge_api_key", "")
            if base_url and model:
                idx = select_best_caption(
                    summary,
                    candidates,
                    base_url=base_url,
                    api_key=api_key,
                    model=model,
                    temperature=judge_config.get("judge_temperature", 0.0),
                    timeout=judge_config.get("judge_timeout", 30),
                )
            else:
                idx = None
            if idx is not None:
        ...
        for caption in candidates:
            score = self.score_caption(summary, caption)
    ...
    def forward(self, inputs,is_eval=False):
        ...
        text_tokens = inputs.get("reference_tokens", None)
        batch_size = point_clouds.shape[0]
        ...
        # build caption conditioning tokens
        num_objs_for_caption = None
        if text_tokens is not None and text_tokens.dim() == 1:
            text_tokens = text_tokens.unsqueeze(0)

        # use query-level features when object-level tokens are provided
        if text_tokens is not None and text_tokens.dim() == 3:
            num_objs_for_caption = text_tokens.shape[1]
            text_tokens = text_tokens.view(-1, text_tokens.shape[-1])
            per_query_feats = box_features[-1].permute(1, 0, 2).contiguous()  # [B, Q, C]
            per_query_feats = per_query_feats[:, :num_objs_for_caption]
            image_tokens = per_query_feats.reshape(-1, 1, per_query_feats.shape[-1])
        else:
            if text_tokens is None:
                text_tokens = torch.zeros((batch_size, 1), dtype=torch.long, device=point_clouds.device)
            text_tokens = text_tokens.view(-1, text_tokens.shape[-1])
            image_tokens = enc_features.permute(1, 0, 2).contiguous()
        if not is_eval:
            ...
            use_autoregressive = inputs.get("use_autoregressive", False)
            tts_config = inputs.get("tts_config", {})
            if use_autoregressive:
                max_len = int(inputs.get("caption_max_len", self.text_seq_len))
                summaries = None
                if tts_config.get("enabled", False):
                    base_summaries = self.build_scene_summary(
                        box_predictions["outputs"],
                        topk=tts_config.get("summary_topk", 10),
                    )
                    if num_objs_for_caption is None:
                        summaries = base_summaries
                    else:
                        summaries = []
                        for b in range(batch_size):
                            summaries.extend([base_summaries[b]] * num_objs_for_caption)
                pred_captions = self.generate_captions_autoregressive(
                    image_tokens=image_tokens,
                    tokenizer=self.tokenizer,
                    max_len=max_len,
                    summaries=summaries,
                    tts_config=tts_config if tts_config.get("enabled", False) else None,
                )
            else:
                ...
                if tts_config.get("enabled", False):
                    candidates = self.sample_captions_from_logits(
                        text_logits_or_embeds,
                        tokenizer=self.tokenizer,
                        num_candidates=tts_config.get("num_candidates", 8),
                        temperature=tts_config.get("temperature", 1.0),
                        top_k=tts_config.get("top_k", 50),
                    )
                    summaries = self.build_scene_summary(
                        box_predictions["outputs"],
                        topk=tts_config.get("summary_topk", 10),
                    )
                    pred_captions = [
                        self.select_tts_caption(summaries[i], candidates[i], tts_config)
            ...
            elif num_decoded % batch_size == 0:
                nqueries = max(1, num_decoded // batch_size)
            else:
                nqueries = nqueries_expected
            ...
            for b in range(batch_size):
                ...
                for q in range(nqueries):
                    ...
                    idx += 1
                    cap_str = self._normalize_caption_for_eval(cap_str)
