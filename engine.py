...
def evaluate_caption(
    ...
    dataset = importlib.import_module(f'datasets.{args.dataset}')
    SCANREFER = getattr(dataset, "SCANREFER", None)
    caption_eval_mode = getattr(dataset, "CAPTION_EVAL_MODE", "scene")
    
    # prepare ground truth caption labels
    print("preparing corpus...")
    gt_corpus = {}
    if caption_eval_mode == "scene":
        for batch in dataset_loader:
            ...
            for i, idx in enumerate(scene_idxs):
                if SCANREFER is not None:
                    scene_name = SCANREFER['scene_list']['val'][idx]
                else:
                    scene_name = batch.get("scene_id", [f"scene_{idx}"])[i]
    ...
    for curr_iter, batch_data_label in enumerate(dataset_loader):
        ...
        model_input = {
            ...
            'caption_max_len': args.max_des_len,
            'tts_config': {
                "enabled": args.use_tts,
                "num_candidates": args.tts_candidates,
                "top_k": args.tts_top_k,
                "temperature": args.tts_temperature,
                "summary_topk": args.tts_summary_topk,
                "judge": args.tts_judge,
                "judge_model": args.tts_judge_model,
                "judge_base_url": args.tts_judge_base_url,
                "judge_api_key": args.tts_judge_api_key,
                "judge_temperature": args.tts_judge_temperature,
                "judge_timeout": args.tts_judge_timeout,
            },
        }
        outputs = model(model_input, is_eval=True)
        box_outputs = outputs.get("box_outputs", outputs)
        captions = box_outputs.get("lang_cap")  # batch, nqueries, [sentence]
        if captions is None:
            captions = outputs.get("lang_cap")
        ...
        if caption_eval_mode == "scene":
            ...
            for i, idx in enumerate(scene_idxs):
                if SCANREFER is not None:
                    ...
                    scene_name = batch_data_label.get("scene_id", [f"scene_{idx}"])[i]
                pred_caption = captions[i][0] if isinstance(captions[i], list) else captions[i]
                predictions[scene_name] = [f'{pred_caption}']
        else:
            # object-level evaluation with IoU gating
            pred_boxes = box_outputs["box_corners"]
            gt_boxes = batch_data_label["gt_box_corners"]
            gt_caps = batch_data_label["gt_box_captions"]
            gt_present = batch_data_label.get("gt_box_present", None)
            scene_ids = batch_data_label.get("scene_id", None)
            if scene_ids is None:
                scene_idxs = batch_data_label["scan_idx"].cpu().tolist()
                scene_ids = [f"scene_{idx}" for idx in scene_idxs]

            def pairwise_iou(pred_corners, gt_corners):
                pred_min, _ = pred_corners.min(dim=1)
                pred_max, _ = pred_corners.max(dim=1)
                gt_min, _ = gt_corners.min(dim=1)
                gt_max, _ = gt_corners.max(dim=1)
                inter_min = torch.max(pred_min[:, None, :], gt_min[None, :, :])
                inter_max = torch.min(pred_max[:, None, :], gt_max[None, :, :])
                inter_dims = torch.clamp(inter_max - inter_min, min=0)
                inter_vol = inter_dims.prod(dim=-1)
                pred_vol = torch.clamp(pred_max - pred_min, min=0).prod(dim=-1)[:, None]
                gt_vol = torch.clamp(gt_max - gt_min, min=0).prod(dim=-1)[None, :]
                return inter_vol / (pred_vol + gt_vol - inter_vol + 1e-8)

            for b in range(pred_boxes.shape[0]):
                pred_caps = captions[b]
                if not isinstance(pred_caps, list):
                    pred_caps = [pred_caps] * pred_boxes.shape[1]
                ious = pairwise_iou(pred_boxes[b], gt_boxes[b])
                for gt_idx in range(gt_boxes.shape[1]):
                    if gt_present is not None and gt_present[b][gt_idx] <= 0:
                        continue
                    best_iou, best_pred = torch.max(ious[:, gt_idx], dim=0)
                    if best_iou < args.test_min_iou:
                        continue
                    key = f"{scene_ids[b]}|{gt_idx}"
                    pred_caption = pred_caps[int(best_pred)]
                    ref_caps = gt_caps[b][gt_idx]
                    if isinstance(ref_caps, str):
                        ref_caps = [ref_caps]
                    gt_corpus[key] = ref_caps
                    predictions[key] = [pred_caption]
        # Memory intensive as it gathers point cloud GT tensor across all ranks
        time_delta.update(time.time() - curr_time)
    ...
    if len(predictions) == 0:
        logout("No valid predictions for caption evaluation.")
        eval_metrics = {
            f"{metric}@{args.test_min_iou}": 0.0
            for metric in ["BLEU-4", "CiDEr", "Rouge", "METEOR"]
        }
        return eval_metrics
