...
def make_args_parser():
    ...
    parser.add_argument(
        "--clip_gradient", default=10, type=float,
        help="Max L2 norm of the gradient"
    )
    parser.add_argument("--warm_lr", default=0.1, type=float)
    ...
    parser.add_argument(
        "--max_des_len", default=77, type=int,
        help="maximum length of object descriptions."
    )
    parser.add_argument("--caption_loss_weight", default=1.0, type=float)
    ...
    parser.add_argument(
        '--vocabulary', default="clib", type=str,
    ...
    parser.add_argument("--eval_every_iteration", default=24000, type=int)
    ...
    parser.add_argument("--use_tts", default=False, action="store_true")
    parser.add_argument("--tts_candidates", default=8, type=int)
    parser.add_argument("--tts_top_k", default=50, type=int)
    parser.add_argument("--tts_temperature", default=1.0, type=float)
    parser.add_argument("--tts_summary_topk", default=10, type=int)
    parser.add_argument("--tts_judge", default="heuristic", type=str, choices=["heuristic", "llm"])
    parser.add_argument("--tts_judge_model", default="gpt-4o-mini", type=str)
    parser.add_argument("--tts_judge_base_url", default="https://api.openai.com/v1/chat/completions", type=str)
    parser.add_argument("--tts_judge_api_key", default="", type=str)
    parser.add_argument("--tts_judge_temperature", default=0.0, type=float)
    parser.add_argument("--tts_judge_timeout", default=30, type=int)

    ##### I/O #####
    parser.add_argument("--checkpoint_dir", default="pretrained_2/Vote2Cap_DETRv4_XYZ_COLOR_NORMAL", type=str)
    parser.add_argument("--log_every", default=10, type=int)

    args = parser.parse_args()
    args.use_height = not args.no_height
    if not args.tts_judge_api_key:
        args.tts_judge_api_key = os.environ.get("TTS_JUDGE_API_KEY", "")
    args.tts_judge_base_url = os.environ.get("TTS_JUDGE_BASE_URL", args.tts_judge_base_url)
    args.tts_judge_model = os.environ.get("TTS_JUDGE_MODEL", args.tts_judge_model)
...
def main(args):
    ...
    # build_scene_graph(datasets,args.checkpoint_dir,split="train")
...
if __name__ == "__main__":
    ...
    if args.use_pretrained is True:
        prefix = f"train_pretrained({args.checkpoint_dir})"
    else:
        prefix = "pretrain"
    wandb.init(project="Vote2Cap", name=f"{prefix}-{time_str}")
