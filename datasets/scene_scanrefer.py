...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if args.vocabulary == 'scanrefer':
            ...
            self.tokenizer = GPT2Tokenizer.from_pretrained('/home/ubuntu/3D/Vote2Cap/gpt')
        elif args.vocabulary == 'clib':
            ...
            if scan_name in self.gathered_language:
                for instance_id in instance_bboxes[:, -1]:
                    if instance_id not in self.gathered_language[scan_name]:
                        ...
                        caption = random.choice(self.gathered_language[scan_name][instance_id])
                        caption_list = list(self.gathered_language[scan_name][instance_id])
                    if self.args.vocabulary == 'scanrefer':
                        caption_text = f"sos {caption} eos" if caption else "sos eos"
                        caption_list = [
                            f"sos {cap} eos" if cap else "sos eos" for cap in caption_list
                        ]
                    else:
                        caption_text = caption
                    object_captions.append(caption_text)
                    gt_box_captions.append(caption_list)
            else:
                object_captions = [''] * len(instance_bboxes[:, -1])
                gt_box_captions = [['']] * len(instance_bboxes[:, -1])

            if self.args.vocabulary == 'clib':
                fixed_length = self.max_des_len
                for i, caption in enumerate(object_captions):
                    token_ids = [self.clip_start_id] + self.tokenizer.encode(caption) + [self.clip_end_id]
                    token_np = np.array(token_ids, dtype=np.int64)
                    if token_np.shape[0] < fixed_length:
                        pad_length = fixed_length - token_np.shape[0]
                        token_np = np.concatenate([token_np, np.zeros((pad_length,), dtype=token_np.dtype)])
                    elif token_np.shape[0] > fixed_length:
                        token_np = token_np[:fixed_length]
                ...
                reference_tokens[:len(object_captions)] = tokens
                reference_masks[:len(object_captions)] = masks

            ret_dict['reference_tokens'] = reference_tokens.astype(np.int64)
            ret_dict['reference_masks'] = reference_masks.astype(np.float32)
            ret_dict['scene_caption'] = object_captions[0] if len(object_captions) > 0 else ''
            if len(gt_box_captions) < MAX_NUM_OBJ:
                gt_box_captions.extend([['']] * (MAX_NUM_OBJ - len(gt_box_captions)))
            ret_dict['gt_box_captions'] = gt_box_captions
